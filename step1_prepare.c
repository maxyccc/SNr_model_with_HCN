...
double calculate_firing_rate(State *restrict s) {
    Spikes spikes = simple_simulation(s, DURATION_init + DURATION_test);
    ...
    for (int i = 0; i < spikes.num_spikes; i++) {
        if (spikes.spike_times[i] >= DURATION_init) {
            first_spike_id = i + 0;
            break;
        }
    }
    ...
    double firing_rate = 1e3 * (spikes.num_spikes - first_spike_id)/(last_spike - first_spike);
    free(spikes.spike_times);
    return firing_rate;
}
...
void setup() {
    ...
    // save result
    char file[128];
    printf("Saving intermediate data at: %s", SAVE_DIR);
...
int main() {
    ...
    printf("Step1 prepare begins \n");
    setup();
    printf("Step1 prepare finishes \n");
